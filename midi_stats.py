...
import sys, os, midi, math, string, time, vmo
...
# the threshold to discard the notes with low intensity (velocity)
Threshold = 10
...
def main():
  ...
  Data = mm.load_train_data('generated_samples.abc')
  print(len(Data))    
  levenshtein_distances = [levenshtein(x, y) for x in Data for y in Data]  
  print 'Levenshtein distance: {}'.format(np.mean(levenshtein_distances))
  
  myPath = 'generated_samples/'
  ...
  if len(allFiles)>0:  
    for i in range(1,len(allFiles)):
      ...
      '''
      ##### Information Rate      
      print('File: {}'.format(allFiles[i]))
      #bars = np.zeros(shape=[88, 16], dtype=np.float)
      source_midi = pretty_midi.PrettyMIDI(myPath+allFiles[i],myTrack=1);
      pianoRoll = source_midi.get_piano_roll(fs=8, times=None);
      pianoRoll = np.clip(pianoRoll[33:121,:],0,127) # to force element to be in [0-127]
      pianoRoll = pianoRoll.transpose()               
            
      r = (0.0, 2.0, 0.01) 
      ideal_v_inv = vmo.find_threshold(pianoRoll, r = r, flag = 'a', dim=pianoRoll.shape[1], dfunc = 'cosine', dfunc_handle = trnspose_inv)
      print(i)
      x_v = np.array([j[1] for j in ideal_v_inv[1]])+0.001
      y_v = [j[0] for j in ideal_v_inv[1]]
      #print(x_v)      
      #print(y_v)      
      ...
      #ax.set_xscale('log')
      plt.tight_layout()
      plt.savefig('irProfile_'+str(i)+'_' +str(max(y_v)/1000)+'.pdf')                           
      #############
      '''
      
      ###### Calculating the other measurements such as scale consistency, uniqueness, velocity span, recurrence, and tone span 
      data = (allFiles[i]).split("_")      
      filename = myPath+allFiles[i]
      ...
      if stats is None:
        ...
        stat = get_gnuplot_line([get_midi_pattern(filename)], 0, showheader=0)        
        '''
        print(i)
        f = open('measurements.txt','a')
        f.write(data[1] + ' ' + data[2] + ' ' + stat)
        f.close()
        '''                
        print ('ML scale estimate: {}: {:.3f}'.format(stats['scale'], stats['scale_score']))
        ...
        print ('Span: {} half-tones, {:.1f} Hz.'.format(stats['tone_span'], stats['freq_span']))
        print ('num_tones: {}'.format(stats['num_tones']))
        ...
        print ('intensity_span (velocity span): {}'.format(stats['intensity_span']))
