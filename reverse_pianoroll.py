...
def piano_roll_to_pretty_midi(piano_rolls, fs=100, program=0):
    ...
    midi_object : pretty_midi.PrettyMIDI
        ...
        the piano roll.
    '''    
    #print(piano_rolls.ndim)
    #raw_input('pr shape in rev')
    if piano_rolls.ndim<3:
        notes, frames = piano_rolls.shape    
        tracks = 1
    else:
        notes, frames, tracks = piano_rolls.shape    
    pm = pretty_midi.PrettyMIDI()
    for track in range(tracks):
        if piano_rolls.ndim<3:
            piano_roll = piano_rolls[:,:]
        else:
            piano_roll = piano_rolls[:,:,track]
        #print(track) 
        #raw_input('track')
        #instrument = pretty_midi.__get_instrument(program, 0, track, 1)
